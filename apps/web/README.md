...
recommendations, the verified Stage 4 explicit-consent saved-personalization flow, and
Stage 5 Phase 6 hybrid/fallback result evidence. It does not imply account authentication
...
in API rank order and scores are not rendered as match percentages. Saved results also
expose server-provided `hybrid` or `stage_4_fallback` mode, typed neutral fallback copy,
and conditional aggregate evidence without client sorting, reweighting, or score
reconstruction.
...
Stage 5 Phase 6 adds the generated personalized-response shape and dedicated saved-result
presentation. The current web suite passes 86 tests together with strict TypeScript,
