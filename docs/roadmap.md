...
1. Python project with pinned direct dependencies and a transitive container
   lock.
...
The stage is complete when migrations, seed, server, tests, linting, and Compose
startup all succeed.

Acceptance was re-audited on 2026-07-26 with 84 fast tests, 28
disposable-PostgreSQL integration tests, 92% diagnostic application coverage, a
clean dependency vulnerability scan, a healthy Compose stack, and the complete
HTTP smoke matrix.
...
8. Harden loading, empty, partial-error, not-found, unavailable, responsive, and
   keyboard states.
...
OpenAPI-drift, accessibility, real-browser, and full-stack Docker gates pass. No
Stage 2 screen may imply that recommendations are active while the model status
remains `not_configured`.

Acceptance was detail-audited with a clean `npm ci`, 40 fast tests, a successful
production build, current OpenAPI-generated contracts, and 21 Docker-first
Playwright scenarios across Chromium, Firefox, and WebKit. The browser suite
passed serious/critical axe checks, keyboard-reachable mobile navigation,
recovery states, URL races, and responsive catalog/detail layouts. The repaired
development stack verified the locked Sharp and PostCSS versions inside its
existing dependency volume. PostgreSQL, API, and web services became healthy,
and the Stage 1 regression suite remained green with 84 fast tests and 28
disposable-PostgreSQL integration tests. See the completion record in the
...
7. Activate an injectable API service with honest unconfigured, unavailable, and
   ready states plus explicit stale-artifact and `catalog_invalid` reasons.
...
the same semantic artifact and ordered rankings; missing, corrupt, incompatible,
or stale artifacts and non-canonical catalog inputs fail clearly; the real API
and web flow pass contract, browser, accessibility, and Docker gates; and all
Stage 1 and Stage 2 regressions remain green.

Onboarding selections remain request-scoped in Stage 3. Persistent preferences,
feedback writes and adjustments, disliked-game exclusion, and model-versioned
recommendation-event logging remain Stage 4 work. The 30-game synthetic catalog
supports functional and reproducibility acceptance, not recommendation-quality
claims.
...
The completion record documents diagnostic size/timing, coverage, licenses, and
the unfixed Debian base-image advisories retained for Stage 7.
...
4. Add bounded atomic preference replacement and canonical like/dislike, played,
   wishlist, and rating state contracts.
...
Stage 4 is complete only when no identity exists before current-version consent;
raw credentials never leave the HttpOnly-cookie boundary; preference and
feedback state is bounded, atomic, idempotent, isolated, and deletable;
...
Stage 4 is complete. The verified worktree passes 184 API tests with 89%
diagnostic coverage, 52 ML tests with 83%, and 76 web tests with 67.15%
...
retention; resources were torn down. Ruff passes across 112 Python files; strict
TypeScript, ESLint, Prettier, production build, generated OpenAPI drift,
...
detailed plan records the completed acceptance evidence. The API Dockerfile now
removes unused Debian `perl-base` after all install steps, resolving the earlier
two critical and two high Perl findings. The rebuilt no-cache
`gamelens-ai-api:stage4-test` image with digest prefix `11b2f940731e` passes
runtime imports and `pip check`, keeps all 49 PostgreSQL integration tests
green, and its comprehensive Docker Scout scan reports 0 critical, 0 high, 3
medium, 27 low, and 2 unspecified findings across 193 packages. Its only-fixed
scan reports no actionable fixed advisory; the remaining base-image findings
stay documented. Bulk key retirement uses the immutable `--created-before`
cohort: quiesce creation/re-consent, drain requests, capture the database-time
cutover while switching issuance to the new secret, revoke until `remaining` is
zero, then retire the old secret. The implementation is published as commit
`c96b6c2` in draft PR [#5](https://github.com/Khang206206/gamelens-ai/pull/5).
...
3. As-of cutoff eligibility, dislike/like/rating/saved-game precedence, stable
   catalog mapping, identity-free canonical profiles, aggregate audit, and typed
   insufficiency/revision errors.
...
6. Fixed-point support pruning, canonical binary CSR, bounded pair counting, raw
   cosine, round-half-up quantization, deterministic top-neighbor selection, and
   canonical sparse storage.
...
13. Frozen `gamelens-hybrid-ranking/1.0.0` contracts define `hybrid` and
    `stage_4_fallback` modes, `content`/`collaborative`/`both` origins, all
    typed unavailable/no-support reasons, active weights, played factor, and the
    full stable tie-break sequence.
...
18. Migrations `0007_stage_5_artifact_registry` through
    ...
    invalidation for authority or included-label loss.
...
20. Saved personalization resolves lifecycle state and invokes the public hybrid
    ranker inside its existing transaction. Every lifecycle/no-support path
    preserves exact Stage 4 ranking output under an explicit fallback reason.
21. Slice commits `0eab24f`, `189ce4c`, `a6d7a34`, `fb7251f`, `4f52547`,
    ...
    removed.
22. Additive saved-response schemas expose mode, fallback, policy/component
    identity, candidate origin, support, source edges, weights, contributions,
    and bounded explanation evidence without changing the stateless contract.
23. Migration `0010_stage_5_event_contract` preserves legacy and Stage 4 rows,
    adds all-or-none Stage 5 mode/component identity and bounded compact result
    constraints, and becomes the expected Alembic head.
24. One pure projector derives response and compact event values from the same
    immutable `PersonalizedRankingDecision`; reconstruction, all 15 fallback
    reasons, deterministic ordering, and top-K/JSON bounds are characterized.
25. `POST /api/v1/me/recommendations` now commits one matching `stage-5-v1`
    event for each acknowledged HTTP 200, commits no event on pre-commit
    failure, preserves ambiguous-outcome semantics, and leaves the stateless
    endpoint unchanged.
26. Generated OpenAPI and TypeScript contracts own the browser shape. The saved
    UI preserves server order, presents hybrid versus neutral fallback mode, and
    reveals aggregate-interaction evidence only when a positive component
    contribution was applied; it does not expose model fingerprints or make
    social-proof/quality claims.
27. Slice commits `8c1c4f9`, `fe784e2`, `c2ddd2d`, `0bbdc58`, `9ab9f68`, and
    `51664b5` complete 6A–6F. The Phase 6 gate passes 365 API unit, 109
    disposable-PostgreSQL, 331 ML (plus one Windows symlink capability skip),
    and 86 web tests. Ruff checks 172 Python files; strict TypeScript, ESLint,
    production build, generated OpenAPI drift, and diff checks pass.
...
Add CI, production Dockerfiles, deployment documentation, logging and monitoring
hooks, security review, and demo assets.

## Stage 8 — Selected advanced capability

**Status:** Planned

Choose only one or two justified capabilities after evaluation is credible, such
as diversity reranking, semantic embeddings, natural-language preference input,
an evidence-grounded LLM explanation layer, or authentication.
